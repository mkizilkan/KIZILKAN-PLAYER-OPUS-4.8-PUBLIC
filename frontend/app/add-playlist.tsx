  ...
  const submit = async () => {
    ...
    try {
      ...
      if (method === "m3u_url") {
        ...
        const channels = await xtreamLiveStreams(cred);
        let vod: any[] = [], series: any[] = [];
        try {
          setProgress("Filmler yükleniyor...");
          vod = await xtVodLocal(cred);
        } catch { /* provider may not have VOD */ }
        try {
          setProgress("Diziler yükleniyor...");
          series = await xtSeriesLocal(cred);
        } catch { /* provider may not have Series */ }
        playlist = {
