 ...
 * Öncelik: tvg-id (varsa en güvenilir) > url > isim.
 * Aynı kanal aynı listede her zaman aynı ID'yi alır.
 */
function channelId(prefix: string, opts: { tvgId?: string | null; url?: string | null; name?: string | null }): string {
  const basis =
    (opts.tvgId && opts.tvgId.trim()) ||
    (opts.url && opts.url.trim()) ||
    (opts.name && opts.name.trim()) ||
    "";
  if (!basis) {
    // Hiçbir tanımlayıcı yoksa son çare rastgele (nadir durum; boş satır vb.)
