...
import { prepareAdultFlags } from '@/src/utils/adult';
...
export function PlaylistProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      prepareAdultFlags(heavy.channels);
      prepareAdultFlags(heavy.vod);
      prepareAdultFlags(heavy.series);
  ...
  const addPlaylist = useCallback(async (p: Playlist) => {
    // +18 bayrağı bir kez hesaplanır; ayardaki toggle anında binlerce metni
    // tekrar taramaz.
    prepareAdultFlags(p.channels);
    prepareAdultFlags(p.vod);
    prepareAdultFlags(p.series);
  ...
  const updatePlaylist = useCallback(async (id: string, patch: Partial<Playlist>) => {
    ...
    if ('channels' in patch) prepareAdultFlags(patch.channels);
    if ('vod' in patch) prepareAdultFlags(patch.vod);
    if ('series' in patch) prepareAdultFlags(patch.series);
